...
def csim():
    if request.method=='POST':
        ...
        ckt=Circuit("desauty")
        ckt.SinusoidalVoltageSource('input','n1',ckt.gnd,amplitude=1@u_V,frequency=100@u_Hz)
        ...
        analysis=sim.transient(step_time=1@u_us,end_time=15@u_ms)
        c=pd.DataFrame(analysis.branches.values())
        cv=float(c.iloc[1,:].max())
        if(4e-9<cv):
            print("current detected",cv)
            cd=1
        else:
            print("current not detected",cv)
            cd=0
            cv=0
        print(round(cv,-7))
        socket.send(cv)
